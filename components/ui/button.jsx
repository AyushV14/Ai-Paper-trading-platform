...
import { cn } from "../../lib/utils" // or "../../lib/utils" if needed based on location
...
const buttonVariants = cva(
  "inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-md text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:size-4 [&_svg]:shrink-0",
  {
    variants: {
      ...
      size: {
        default: "h-10 px-4 py-2", // picked the taller default from both
        sm: "h-8 rounded-md px-3 text-xs", // text-xs from one of your versions
        lg: "h-11 rounded-md px-8", // taller lg from one version
        icon: "h-10 w-10", // bigger icon version
      },
    },
    defaultVariants: {
      variant: "default",
